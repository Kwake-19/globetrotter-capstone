...
// Relocated from the monolith's src/routes/search.routes.js. Only change:
// the destination list comes from destinations-service over HTTP. Uses Groq
// (if configured) to turn the query into a { category, keywords } filter,
// then matches that against the real destinations - the AI only ever
// interprets intent, it never invents results.
router.get('/', async (req, res, next) => {
  try {
    ...
    if (!process.env.GROQ_API_KEY) {
      const results = plainSubstringSearch(destinations, q);
    ...
    const results = rankByUnderstanding(destinations, understood);
