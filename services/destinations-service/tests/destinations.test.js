...
describe('Destinations', () => {
  ...
  it('filters by category', async () => {
    ...
    res.body.results.forEach((place) => expect(place.category).toBe('mall'));
  });
  ...
  it('always includes placeId and localImagePath, as null or a string', async () => {
    const list = await request(app).get('/api/destinations');
  ...
  describe('GET /api/destinations/nearby', () => {
    ...
    it('returns results sorted by distance ascending, each with a distanceKm field', async () => {
      ...
      res.body.results.forEach((place) => expect(typeof place.distanceKm).toBe('number'));
      for (let i = 1; i < res.body.results.length; i += 1) {
    ...
    it('excludes places outside radiusKm and includes them once the radius is widened', async () => {
      ...
      expect(widerRadius.body.results.length).toBeGreaterThanOrEqual(defaultRadius.body.results.length);
    });
