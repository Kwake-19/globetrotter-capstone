...
// and the frontend chooses where to store it (localStorage vs
// sessionStorage) to match. See public/js/api.js's setAuth/getToken.
//
// Phase 2 change: the token payload now carries `isAdmin` so the API
// gateway can authorize admin routes straight from the token, without
// looking a user record up on every request. Trade-off: granting someone
// admin (a manual data edit) only takes effect on their next login.
function signToken(user, rememberMe) {
  const expiresIn = rememberMe
    ? (process.env.JWT_REMEMBER_EXPIRES_IN || '30d')
    : (process.env.JWT_EXPIRES_IN || '7d');
  return jwt.sign(
    { sub: user.id, name: user.name, email: user.email, isAdmin: !!user.isAdmin },
