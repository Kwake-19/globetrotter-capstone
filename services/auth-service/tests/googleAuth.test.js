...
// Never call real Google servers - replace the OAuth2Client constructor
// with one whose verifyIdToken is a jest.fn() each test controls. Same
// approach as the Phase 1 monolith's googleAuth test.
...
describe('Google Sign-In', () => {
  ...
  describe('POST /api/auth/google', () => {
    ...
    it('creates a new user from a first-time Google sign-in', async () => {
      ...
      expect(jwt.decode(res.body.token).isAdmin).toBe(false);
    });
    ...
    it('respects rememberMe for the issued token lifetime', async () => {
      mockVerifyIdToken.mockResolvedValueOnce(googleTicket({
