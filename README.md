...
container's port 4001 (visit `http://localhost:4001` — see
...
Note: `npm run dev` (local, no Docker) also defaults to port 4001, so the
two can't run at the same time — stop one before starting the other.
...
`render.yaml` deploys this repo to [Render](https://render.com) as a Docker
web service:

1. Push this repo to GitHub.
2. In the Render dashboard: **New +** → **Blueprint**, point it at the repo.
   Render reads `render.yaml` and builds `Dockerfile` automatically.
3. `JWT_SECRET` is auto-generated by the blueprint. `GOOGLE_PLACES_API_KEY`
   and `GOOGLE_MAPS_EMBED_KEY` are optional — set them in the Render
   dashboard's Environment tab if you want photos/maps live in production.

**Important caveat**: Render's free tier has an **ephemeral filesystem** —
`data/db.json` resets to the seed destinations (baked into the image) on
every redeploy, restart, or scale-to-zero. Fine for a demo; any real
signups/itineraries created at runtime won't survive a restart. To actually
persist data for free, the alternative is a self-managed VM with real
persistent disk (e.g. [Oracle Cloud's Always Free
tier](https://www.oracle.com/cloud/free/)) — you SSH in, install Docker, and
