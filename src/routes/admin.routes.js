...
router.post('/destinations', async (req, res, next) => {
  try {
    ...
    const destination = {
      ...
      photos,
      // Read-only convenience field for existing frontend code that
      // still reads a single localImagePath - always derived from
      // photos[0], never set directly from the admin form.
...
router.put('/destinations/:id', async (req, res, next) => {
  try {
    ...
    if (body.photos !== undefined) {
      ...
      // Kept in sync automatically - the admin form edits photos, never
      // this field directly.
